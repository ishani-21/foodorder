...
<div class="main-panel">
    <div class="content-wrapper">
        <form class="forms-sample" method="POST" enctype="multipart/form-data" action="{{route('admin.subcategory.store')}}">
            @csrf
            <div class="form-group">
                <label for="categorys_id">Category</label>
                    ...
                    @foreach($category as $category)
                        <option class="dropdown-item" value="{{ $category->id }}"{{ $category->id == old('categorys_id') ? 'selected' : '' }}>{{ $category->name }}</option>
                    @endforeach 
                    </select>
            </div>
            @error('categorys_id')
